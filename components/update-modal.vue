<template>
  <div class="fixed top-0 left-0 bg-black bg-opacity-75 p-2 pl-16 w-full h-screen">
    <i @click="$emit('toggleUpdate')" class="fa-solid fa-xmark absolute md:text-xl cursor-pointer top-4 right-4 rounded-full w-6 h-6 md:w-10 md:h-10 flex justify-center items-center bg-white md:hover:bg-gray-400 md:hover:text-white md:transition md:duration-200"></i>
    ...
    <div class="mx-auto md:w-4/12 p-2 my-1 bg-blue-500 rounded-lg">
      <form @submit.prevent class="relative flex flex-col gap-2 text-white bg-blue-500 rounded-lg">
        ...
        <button @click.prevent="updateCoupon" type="button" class="w-full md:text-xl mt-1 px-4 py-2 rounded-lg text-white font-semibold bg-green-400 md:hover:bg-green-300 md:active:bg-green-500 md:transition md:duration-200">Actualizar cupon</button>
      </form>
    </div>
  </div>
</template>
